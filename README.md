...
GPU Indirect Drawing 기반의 대규모 식생 렌더링 최적화 및 절차적 자동 배치 툴 세트입니다.
기존 프리팹/브러시 배치 방식의 한계를 극복하고, 수십만 개의 식생을 모바일 환경에서도 고성능으로 렌더링·편집할 수 있도록 에디터 및 런타임 시스템을 전면 개편한 포스트모템 기반 개선 프로젝트입니다.

## 사용 설명서

툴을 사용하기 전에 [기술_매뉴얼.pdf](./기술_매뉴얼.pdf)를 먼저 읽어주세요.

Grass Builder의 DensityMap 베이킹 절차, Foliage Day 브러시 단축키, 팔레트 구성 및 데이터 저장 방식 등
아래 요약만으로는 알 수 없는 실제 작업 순서와 주의사항이 이 문서에 정리되어 있습니다.
설명서의 절차를 건너뛰고 툴을 사용할 경우 DensityMap 누락이나 식생 데이터 손실이 발생할 수 있습니다.
...
- BVH (Bounding Volume Hierarchy) 공간 분할 도입
  ...
  - BVH 적용으로 200여 회의 Raycasting 연산 시간을 10ms 이하로 단축 (30 FPS 기준 안정적 동작 확보).
- 식생 데이터 전용 Quadtree 기법 적용
  - 수십만 개 식생 탐색 시 프레임 저하를 방지하기 위해 위치 기반 Quadtree 적용.
  - 중간 삽입/삭제 알고리즘을 추가하여 브러시 작업 중 지속적인 트리 재구성 오버헤드 최소화.
...
- 실험 결과 (Galaxy S22 Ultra 기준, 식생 30만 개 테스트)
  - FPS: 기존 5 FPS 내외에서 25 FPS로 개선 (플레이 가능 수준 확보).
  - 메모리: 기존 대비 약 800MB 절감.
  - Depth Priming 도입으로 Cardboard 방식 반투명 식생의 Overdraw 이슈 개선.
